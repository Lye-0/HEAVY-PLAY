...
export class Playground {
 constructor(){
  ...
  this.createWorld();this.input=new Interaction(this);this.bindUI();this.updateCount();this.updateState();
  ...
  let resizeTimer;window.addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>this.renderer.resize(),70);});
 ...
 createWorld(){this.world=new World(PRESETS[this.quality].count,417);const snapshot=SNAPSHOTS[String(PRESETS[this.quality].count)];if(snapshot)this.world.applySnapshot(snapshot);this.world.onImpact=(s,b)=>this.audio.impact(s,b);this.baseSnapshot=this.world.snapshot();}
 bindUI(){
  ...
  $('sound').addEventListener('click',async()=>{const button=$('sound');button.disabled=true;try{const on=await this.audio.toggle();button.setAttribute('aria-pressed',String(on));button.querySelector('use').setAttribute('href',on?'#i-sound':'#i-mute');button.querySelector('span').textContent=on?'SOUND ON':'SOUND OFF';button.setAttribute('aria-label',on?'金属音をオフにする':'金属音をオンにする');button.title=button.getAttribute('aria-label');if(on){this.audio.impact(4,this.world.bodies[0]);this.toast('SOUND ON — 金属の響きも、楽しんで。');}}catch(e){this.toast(e.message||'音声を開始できませんでした。');}finally{button.disabled=false;}});
  ...
  document.addEventListener('pointerdown',e=>{if(!$('settings').hidden&&!e.target.closest('#settings,#settings-toggle'))this.toggleSettings(false);});
 ...
 blast(point,power=1){if(this.rebuilding)return;this.paused=false;this.updateState();this.interact();this.world.blast(point,power,4.5+power*1.4);this.waves.push({p:[point[0],.035,point[2]],start:this.time,strength:power});this.audio.impact(10*power,this.world.bodies[0]);}
 ...
 reset(){if(this.rebuilding)return;this.input.clear();this.dropQueue=0;this.waves=[];this.paused=false;this.accumulator=0;const count=PRESETS[this.quality].count;this.world.bodies.length=count;
  ...
  this.rebuilding={start:this.time,from:this.world.bodies.map(b=>({p:b.p.slice(),q:b.q.slice()}))};for(const b of this.world.bodies){b.v.fill(0);b.w.fill(0);}this.interact();this.setActionDisabled(true);this.updateCount();this.updateState();this.toast('ひとつずつ、元の山へ。');
 }
 ...
 openHelp(){this.toggleSettings(false);this.input.clear();if(!$('help').open)$('help').showModal();this.interact();}
 ...
 frame(now){
  ...
  const rawDt=Math.max(.001,(now-this.lastFrame)/1000),dt=Math.min(rawDt,.045);this.lastFrame=now;this.time+=dt;this.frames++;this.input.tick(now);
